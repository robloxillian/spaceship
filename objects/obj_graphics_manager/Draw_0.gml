...
if (obj_player.lung_oxygen < 100) {
	var red_alpha = 1 - (obj_player.lung_oxygen/100);
...
if (obj_player.is_dying) {
	if (death_message_chars > 0) {
		//var end_message_chars_1 = min(end_message_chars, 7); //TODO: globalize the 7
		var display_text_1 = string_copy(global.DEATH_MESSAGE, 1, death_message_chars);
		draw_set_font(font_end);
		draw_set_color(c_black);
		draw_text(cx + cw/2 - 40, cy + cy/2 + 60, display_text_1);
	}
	
} else if (obj_danger_event_controller.game_is_ending) {
	...
	if (end_message_chars > 0) {
		...
		if (end_message_chars >= 9) {
			...
			draw_text(cx + cw/2 - 40, cy + cy/2 + 70, display_text_2);
		}
	}
}
